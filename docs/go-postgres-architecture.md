...
Agent 完整输出写入 `DATA_DIR/logs`。数据库事件只保存 `agent-run:<run-id>` 日志引用和累计字节数，不复制完整输出或密钥。
...
SSE 地址为 `/api/v1/events/stream?projectId=<uuid>`。PostgreSQL `events.id` 同时作为 SSE `id`，客户端可通过 `Last-Event-ID` 回放断线期间的事件。
