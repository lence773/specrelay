...
import type { FeedbackNavigationTarget } from "./features/feedback/FeedbackViews";
  ...
  return (
    <div className="window-controls">
      <button
        ...
        title="最小化"
        aria-label="最小化"
        onClick={() => void invokeDesktop("minimize_window")}
      >
        −
      </button>
      <button
        ...
        title="最大化或还原"
        aria-label="最大化或还原"
        onClick={() => void invokeDesktop("toggle_maximize_window")}
      >
        □
      </button>
      <button
        ...
        title="关闭"
        aria-label="关闭"
        onClick={() => void invokeDesktop("close_window")}
      >
        ×
      </button>
    </div>
...
export function App() {
  ...
  const [focus, setFocus] = useState<FeedbackNavigationTarget>();
  ...
  useEffect(() => {
    if (!selected && projects.data?.[0]) setSelected(projects.data[0].id);
  }, [projects.data, selected]);
  const project = projects.data?.find((p) => p.id === selected);
  const intakes = useQuery({
    queryKey: ["intakes", selected],
    queryFn: () => api.intakes(selected!),
    enabled: !!selected,
  });
  const plans = useQuery({
    queryKey: ["plans", selected],
    queryFn: () => api.plans(selected!),
    enabled: !!selected,
  });
  const eventPage = useQuery({
    queryKey: ["events", selected],
    queryFn: () => api.events(selected!, undefined, EVENT_PAGE_LIMIT),
    enabled: !!selected,
  });
  useEffect(() => {
    setRecentEvents([]);
  ...
  useEffect(() => {
    if (eventPage.data)
      setRecentEvents(
        eventPage.data.items.filter((event) => event.type !== "agent.output"),
      );
  }, [eventPage.data]);
  useEffect(() => {
    if (!selected || !eventPage.isSuccess) return;
    const after =
      eventPage.data.items.find((event) => event.type !== "agent.output")?.id ??
      0;
    return subscribe(selected, after, queryClient, (event) =>
  ...
  const navigateToFeedbackTarget = (target: FeedbackNavigationTarget) => {
    setFocus(target);
    setTab(
      target.kind === "intake" || target.kind === "feedback"
        ? "intakes"
        : "plans",
    );
  };
  const automation = useMutation({
    mutationFn: (enabled: boolean) => api.automation(project!, enabled),
  ...
  const counts = useMemo(
    () => ({
      intakes:
        intakes.data?.filter(
          (i) => i.status === "open" || i.status === "plan_failed",
        ).length ?? 0,
      plans:
        plans.data?.filter(
          (p) => p.status === "running" || p.status === "blocked",
        ).length ?? 0,
    }),
  ...
  if (!authReady || projects.isLoading)
    return (
      <div className="splash">
        <div className="splash-logo">S</div>
        <span>正在启动 SpecRelay…</span>
      </div>
    );
    ...
    <div className="app-shell">
      ...
      <main className={project ? "workspace-main" : undefined}>
        <header className={`topbar ${project ? "" : "empty-topbar"}`}>
            ...
            <nav>
              {nav.map(([id, label, icon]) => (
                ...
                >
                  {icon}
                  <span>{label}</span>
                  {id === "intakes" && counts.intakes > 0 && (
                    <b>{counts.intakes}</b>
                  )}
                  {id === "plans" && counts.plans > 0 && <b>{counts.plans}</b>}
                </button>
              ))}
            </nav>
          ) : (
          ...
          {project && (
            <button
              className={`automation ${project.automationEnabled ? "running" : ""}`}
              disabled={automation.isPending}
              title={
                project.automationEnabled
                  ? "停止后会取消排队中的自动任务"
                  : "启动后会自动生成计划，并执行所有已就绪计划"
              }
        ...
        {project ? (
          <div className="content">
            {tab === "overview" && (
              <Overview
                ...
                plans={plans.data ?? []}
                events={recentEvents}
                onNavigate={(value) => setTab(value as Tab)}
              />
            )}
            <div hidden={tab !== "intakes"}>
              <IntakesView
                key={project.id}
                project={project}
                intakes={intakes.data ?? []}
                focus={focus}
                onNavigate={navigateToFeedbackTarget}
              />
            </div>
            {tab === "plans" && (
              <PlansView
            ...
            {tab === "events" && (
              <EventsView
                events={[...(eventPage.data?.items ?? [])].reverse()}
              />
            )}
          ...
          <div className="welcome">
            <Empty
              title="创建第一个项目"
              body="将 SpecRelay 绑定到可信的本地工作目录，配置智能体，然后把需求转化为经过验证的代码。"
              action={
                <button
