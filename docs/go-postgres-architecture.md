...
SpecRelay **没有云端遥测**。结构化观测数据只写入用户在桌面端或环境变量中配置的 PostgreSQL；CLI 原始输出只写入后端所在机器的 `DATA_DIR/logs`。观测查询不会读取日志文件，也不会调用 Codex、Claude、第三方计费接口或任何 SpecRelay 云服务。数据库和日志目录的备份、保留期、访问控制与跨境策略均由用户自己的部署环境决定。

`agent_runs` 以一次物理 CLI/validation 调用为一行，观测白名单包含：

- 关联关系：项目、需求、计划、任务、`logical_operation_id`；
- 调用分类：`operation_type`、Provider、作业尝试次数、重试次数、会话模式；
- 结果分类：状态、退出码、失败类别、开始/结束时间；
- 性能与输出规模：排队耗时、运行耗时、输出字节/行数、结构化事件数、是否截断；
- Provider 明确上报的用量：输入/输出/总 Token、费用数值和币种。

下列字段即使存在于运行表中也不进入观测查询或默认导出：工作区路径、完整 CLI 会话 ID、命令和参数、环境变量、日志路径与正文、原始终止/错误正文、提示词、源码或附件内容。API 返回和导出都使用显式字段白名单，避免新增内部列后被自动暴露。

聚合规则如下：

1. 时间边界按物理调用的 `started_at` 过滤，`from` 与 `to` 均为包含边界；Provider、计划和项目条件与时间条件同时生效。
2. 会话复用率和快照恢复率以会话模式为 `new`、`reused` 或 `snapshot_restored` 的物理调用为分母；`not_applicable` 与未知旧数据不进入分母。
3. 计划生成和任务执行成功率按 `operation_type + logical_operation_id` 去重。重试产生的多次物理调用归入同一逻辑操作；任一次最终成功则该逻辑操作记为成功，否则使用最后一次终态及失败类别。
4. 排队和运行耗时只聚合非空值，并同时返回覆盖调用数；无覆盖时平均值不可用。
5. Token 只累加 Provider 明确返回的字段。字段缺失、未知格式、超大/截断结构化事件或不支持的 Provider 均保持不可用，不从文本长度、输入输出之和或价格表推算。明确上报的零 Token 是可用的零值。
6. 费用只汇总同时具有数值和币种的调用，按币种分别分组并返回各自覆盖数；不换算汇率，不把未知费用记为零。
7. 没有匹配数据时，列表和关联数组为空，成功率、耗时、Token 与费用均返回 `available=false`，且不伪造数值字段。

观测列表默认每页 50 条，API 允许 1–200 条；聚合基于全部筛选结果，分页只切分物理调用列表。运行日志接口与观测接口相互独立：日志默认读取最新 50 条，使用排他的字节游标向上加载更早 50 条。

JSON 与 CSV 导出应用相同的项目和筛选条件，并表达同一组运行、成功率、失败类别、耗时趋势、用量覆盖和分币种费用。默认导出还会移除项目名称、工作区路径以及需求/计划/任务业务标题。只有用户显式勾选对应选项并在界面再次确认后，才会包含所选项目元数据；即使显式选择元数据，完整会话 ID、命令参数、日志正文、源码片段和疑似 Token 仍不属于可导出字段。
